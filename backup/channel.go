...
func newChannel(topic string, params map[string]interface{}, socket *Socket) *Channel {
	...
	// Set up built-in event handlers (without locking during construction)
	ch.bindingRef++
	...
	ch.bindings = append(ch.bindings, EventBinding{
		...
		callback: func(payload interface{}) {
			...
			ch.mu.Lock()
			if ch.state == ChannelJoining && ch.joinPush != nil {
				ch.joinPush.Reset()
			}
...
func (ch *Channel) Join(timeout ...time.Duration) *Push {
	...
	// Perform the rejoin logic without holding the lock
	if ch.state != ChannelLeaving {
		...
		// Perform socket operations without holding channel locks
		ch.socket.leaveOpenTopic(topic)
		newJoinRef := ch.socket.MakeRef()

		// Update joinRef with proper locking
		ch.mu.Lock()
		ch.joinRef = newJoinRef
		ch.mu.Unlock()

		// Call Resend without holding the channel lock to avoid deadlock
...
func (ch *Channel) Leave(timeout ...time.Duration) *Push {
	...
	// Set up callbacks and send without holding the channel lock
