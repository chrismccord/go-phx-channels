...
func (p *Push) startTimeout() {
	...
	// Register reply event handler - prepare callback first
	replyCallback := func(payload interface{}) {
		go func() {
			p.mu.Lock()
			defer p.mu.Unlock()
	...
	// Register without holding the push lock to avoid deadlock
	channel := p.channel
	refEvent := p.refEvent
	p.mu.Unlock()
	refEventRef := channel.On(refEvent, replyCallback)
	p.mu.Lock()
	p.refEventRef = refEventRef
