...
func TestNewChannel(t *testing.T) {
	...
	assert.Len(t, channel.bindings, 3) // phx_reply, phx_close, phx_error handlers
...
func TestChannelJoin(t *testing.T) {
	...
	// Mock socket as connected but without actual connection
	socket.mu.Lock()
	socket.connected = false // Keep as false to avoid trying to send over nil connection
	socket.state = StateClosed
	socket.mu.Unlock()
